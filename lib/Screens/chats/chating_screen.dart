          ...
          leadingIcon: Row(
            ...
            children: [
              IconButton(
                ...
                onPressed: () => Get.back(),
                icon: Icon(Icons.arrow_back_ios_outlined, color: AppColors.blackColor, size: 14),
              ),
              CachedImageCircle2(
                isCircular: true,
                height: 40,
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: IconButton(
                            ...
                            onPressed: () {
                              if (logic.msgCtrl.text.isNotEmpty || logic.attachments.isNotEmpty) {
                                logic.sendMessage(
                  ...
                  if (logic.isDataLoading)
                    ...
                    ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isMyMessage = msg.senderId == AuthData().userModel?.id;
                        final alignment = isMyMessage ? Alignment.centerRight : Alignment.centerLeft;
                        final color = isMyMessage ? AppColors.whiteColor : AppColors.blueColor;
                        final file = msg.attachments?.isNotEmpty == true ? msg.attachments![0] : null;
                        final ext = file?.fileType?.toLowerCase() ?? '';

                        Widget? preview;
                        if (file != null) {
                          if (ext.startsWith('image') || ['jpg', 'jpeg', 'png', 'gif'].contains(ext)) {
                            preview = ClipRRect(
                            ...
                            IconData? icon;
                            Color color = isMyMessage ? Colors.blue : Colors.white;
                            if (ext.contains('pdf')) icon = Icons.picture_as_pdf;
                            else if (ext.contains('mp4')) icon = Icons.videocam;
                            else if (ext.contains('mp3')) icon = Icons.audiotrack;
                            else if (['doc', 'docx'].contains(ext)) icon = Icons.description;
                            else if (['xls', 'xlsx'].contains(ext)) icon = Icons.grid_on;
                            else if (['ppt', 'pptx'].contains(ext)) icon = Icons.slideshow;
                            else if (ext == 'txt') icon = Icons.notes;
                            if (icon != null) {
                              preview = GestureDetector(
                                onTap: () => logic.openNetworkFile(file.url ?? ''),
                                  ...
                                  children: [
                                    ...
                                    if (msg.messageType?.toLowerCase() == 'offer')
                                      Row(
                                        mainAxisSize: MainAxisSize.min,
                                        children: [
                                          Image.asset(AppAssets.couponAppliedLogo, height: 20, width: 20, color: Colors.deepOrange),
                                          addWidth(6),
                                          Text('Offer', style: TextStyle(fontWeight: FontWeight.w600, color: Colors.deepOrange)),
                                        ],
                                      ).marginOnly(bottom: 20),
                                    if (msg.message?.isNotEmpty == true)
                                      Text(msg.message!, style: TextStyle(color: AppColors.blackColor)),
                                  ],
                                ),
                              ),
                            ),
