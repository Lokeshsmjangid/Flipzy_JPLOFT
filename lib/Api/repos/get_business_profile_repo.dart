...
Future<CommonModelResponse> businessProfileApi() async {
  ...
  try{
    ...
    flipzyPrint(message: '${ApiUrls.bussinessProfileUrl},$map');
    http.Response response = await performPostRequest(ApiUrls.bussinessProfileUrl,map);
    var data = json.decode(response.body);

    if (response.statusCode == 200) {
      log("${ApiUrls.bussinessProfileUrl}\n $map \n response Start-->\n\n $data \n\n<--response End" );
      return CommonModelResponse.fromJson(data);
    } else {
      handleErrorCases(response, data, ApiUrls.bussinessProfileUrl);
    }
  }
